  ...
  describe("Staker", function () {
    ...
    describe("mintItem()", function () {
      it("Balance should go up when you stake()", async function () {
        ...
        // .to.emit(owner.address, 'You have just staked with us');
        ...
        await expect(stakeResult).to.emit(stakerContract, "LogStake");
      });
      ...
      if (process.env.CONTRACT_ADDRESS) {
        ...
        it("Should redeploy Staker, stake, not get enough, and withdraw", async function () {
          ...
          console.log('\t', " 🎉 calling execute")
          // const execResult = await stakerContract.execute();
          await expect(stakerContract.execute()).to.be.revertedWith("threshold not reached");
        ...
        it("should be able to withdraw if amount not 1 ether", async () => {
          const [owner] = await ethers.getSigners();

          console.log('\t', " 🔨 Staking...")
          const stakeResult = await stakerContract.stake({ value: ethers.utils.parseEther("0.1") })

          const txResult = await stakeResult.wait()
          expect(txResult.status).to.equal(1);

          const balAfterStaking = await stakerContract.balances(owner.address);
          expect(balAfterStaking).to.equal(ethers.utils.parseEther("0.1"));
          console.log("\n\tbalance after staking", ethers.utils.formatEther(balAfterStaking));
          ...
          //test withdrawal
          await stakerContract.withdraw();
          const balAfterStaking2 = await stakerContract.balances(owner.address);
          expect(balAfterStaking2).to.equal(0);
          console.log("\n\tbalance after withdrawal", ethers.utils.formatEther(balAfterStaking2));
        });
      }
      //
